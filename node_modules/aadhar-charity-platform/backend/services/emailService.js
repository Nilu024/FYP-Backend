...
const sendAdminVerificationEmail = async ({ type, title, createdBy, entityId }) => {
  const adminEmail = process.env.ADMIN_EMAIL;
  if (!adminEmail) {
    console.warn("ADMIN_EMAIL is not set. Skipping admin verification email.");
    return false;
  }

  const body = `
    <p class="body-text">Hello Admin,</p>
    <p class="body-text">
      A new ${type} has been submitted and is currently <b>pending verification</b>.
    </p>

    <div class="info-box">
      <p class="body-text"><strong>Type:</strong> ${escapeHtml(type)}</p>
      <p class="body-text"><strong>Title:</strong> ${escapeHtml(title || "N/A")}</p>
      <p class="body-text"><strong>Submitted By:</strong> ${escapeHtml(createdBy?.name || "N/A")} (${escapeHtml(createdBy?.email || "N/A")})</p>
      <p class="body-text"><strong>Entity ID:</strong> ${escapeHtml(entityId || "N/A")}</p>
    </div>

    <p class="body-text">Please review in the Admin panel.</p>
  `;

  return sendEmail({
    to: adminEmail,
    subject: `Verification needed: ${type}${title ? ` - ${title}` : ""}`,
    html: baseTemplate({
      title: "Admin Verification Alert",
      preheader: "New submission requires review",
      body,
    }),
    text: `New ${type} pending verification. Title: ${title}. ID: ${entityId}.`,
  });
};
