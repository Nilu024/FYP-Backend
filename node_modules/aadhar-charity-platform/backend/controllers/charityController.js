  ...
  try {
    ...
    if (existing) {
      return res.status(400).json({
        success: false,
        error: "You already have a registered charity",
      });
    }
...
exports.toggleFollow = async (req, res, next) => {
  try {
    ...
    if (isFollowing) {
      user.followedCharities = user.followedCharities.filter(
        (id) => id.toString() !== charity._id.toString(),
      );
      charity.followers = charity.followers.filter(
    ...
    res.json({
      ...
      followerCount: charity.followerCount,
    });
  } catch (err) {
    next(err);
  }
...
exports.getMyCharity = async (req, res, next) => {
  try {
    ...
    if (!charity)
      return res
        .status(404)
        .json({ success: false, error: "No charity found for your account" });
    res.json({ success: true, data: charity });
  } catch (err) {
    next(err);
  }
