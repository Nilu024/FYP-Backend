  ...
  try {
    ...
    if (!charity) {
      return res
        ...
        .json({
          success: false,
          error: "No charity associated with your account",
        });
    }
    ...
    if (!charity.isVerified) {
      return res
        ...
        .json({
          success: false,
          error: "Your charity must be verified before listing needs",
        });
    }
...
exports.updateNeed = async (req, res, next) => {
  try {
    ...
    if (
      need.charity.owner.toString() !== req.user.id &&
      req.user.role !== "admin"
    ) {
  ...
  try {
    ...
    const need = await Need.findByIdAndUpdate(
      req.params.id,
      {
        ...
        verifiedAt: ["approved", "rejected"].includes(status)
          ? new Date()
          : undefined,
    ...
    if (status === "approved" && need.charity) {
      ...
      await notifyCharityFollowers(need.charity._id, {
        ...
        data: {
          ...
          charityId: need.charity._id.toString(),
          url: `/needs/${need._id}`,
        },
      });
...
exports.getNeedsByCharity = async (req, res, next) => {
  try {
    ...
    const isOwnerOrAdmin =
      req.user && (req.user.role === "admin" || req.user.role === "charity");
